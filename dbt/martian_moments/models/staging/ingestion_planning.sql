...
WITH rover_gaps AS (
    SELECT 
        ...
        sol_gap
    FROM 
        {{ ref('validation_rover_gaps') }}
),

sol_ranges AS (
    SELECT 
        rover_name,
        actual_max_sol as start_sol,
...
ingestion_tasks AS (
    SELECT 
        ...
        CASE 
            WHEN 
                total_sols_needed >= 100 THEN (CEIL(start_sol / 100.0) * 100) + 1
            ELSE 
                start_sol + 1
        END as start_sol,
        LEAST(CEIL((start_sol + 100) / 100.0) * 100, max_sol) + 1 as end_sol,
