...
def fetch_next_ingestion_batch(run_dbt_models_success, logger):
    if run_dbt_models_success:
        ...
        try: 
            ...
            rows = snowflake_cursor.execute(f"SELECT rover_name, sol FROM VALIDATION_PHOTO_GAPS WHERE validation_status = 'MISSING_SOL' ORDER BY sol LIMIT {BATCH_SIZE}").fetchall()
